% FUNCTION printFig( fileName, [theLineWidth] )
%
...
%
% The parameter theLineWidth sets the width of the lines (default 1).
%
% Last modified by Andreas Jakobsson, 140116.
%
function printFig( fileName, theLineWidth )
...
for i=1:length(ha)
  hc = get(ha(i),'Children');
    for j=1:length(hc)
        set(hc(j),'LineWidth',theLineWidth);
    end
end
